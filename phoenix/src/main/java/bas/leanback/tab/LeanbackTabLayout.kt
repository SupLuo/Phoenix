 ...
 *
 *  备注：
 *      TabLayout和ViewPager的联动，应该让Tab去处理ViewPager的切换，在ViewPager的切换回调中让TabLayout选中对应位置
 *
 *      TabLayout的item选中之后，会改变tabview的isSelected属性，
 */
class LeanbackTabLayout @JvmOverloads constructor(
    ...
    var viewPagerSmoothScroll: Boolean = true
        set(value) {
            mediator?.setViewPagerSmoothScroll(value)
            field = value
        }
            ...
            if (duplicateTabViewState && ::duplicateTabViewStateListener.isInitialized) {
                it.onFocusChangeListener = ChainFocusListener(
                    TabFocusChangeListener(
                        this, viewPager, viewPagerSmoothScroll
