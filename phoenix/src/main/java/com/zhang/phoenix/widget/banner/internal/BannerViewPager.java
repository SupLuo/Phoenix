package com.zhang.phoenix.widget.banner.internal;
...
/**
 * 复制banner库中的控件，修改adapter的引用，目前此库代码写的非常死板
 */
...
public class BannerViewPager<T> extends RelativeLayout implements LifecycleObserver {
    ...
    private void initView() {
        inflate(getContext(), com.zhpan.bannerview.R.layout.bvp_layout, this);
        mViewPager = findViewById(com.zhpan.bannerview.R.id.vp_main);

        mViewPager.setFocusable(false);
        mViewPager.setFocusableInTouchMode(false);

        mIndicatorLayout = findViewById(com.zhpan.bannerview.R.id.bvp_layout_indicator);
        mViewPager.setPageTransformer(mBannerManager.getCompositePageTransformer());

        mIndicatorLayout.setFocusable(false);
        mIndicatorLayout.setFocusableInTouchMode(false);
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        shouldIntercept = super.onInterceptTouchEvent(ev);
        if(!shouldIntercept){
            isLooping = false;
            startLoop();
        }
